...
# Based on plugins and functions from many many different people. See function comments and requirements for details.
...
def TweakDarks(src, s0=2., c=0.0625, chroma=True):
...
def vs_temporalfix(clip, strength=400, tr=6, exclude=None, debug=False):
    ...
    D6 = core.mv.Degrain6
    bd = 16
    i = 0.00392 if mvtr > 6 else 1 << (bd - 8)
    ...
    motionmask_shift_minus_one   = motionmask[1:]  + motionmask[-1]
    motionmask_shift_minus_two   = motionmask[2:]  + motionmask[-2]
    motionmask_shift_minus_three = motionmask[3:]  + motionmask[-3:]
    motionmask_shift_plus_one    = motionmask[0]   + motionmask[:-1]
    motionmask_shift_plus_two    = motionmask[0:2] + motionmask[:-2]
    motionmask = core.std.Expr([motionmask, motionmask_shift_minus_one, motionmask_shift_minus_two, motionmask_shift_minus_three, motionmask_shift_plus_one, motionmask_shift_plus_two],expr=['x y + z 0.75 * + a 0.5 * + b 0.75 * + c 0.5 * +'])
...
def ContraSharpening(clip, src, radius=None, rep=24, planes=[0, 1, 2]):
    #function from G41Fun https://github.com/Vapoursynth-Plugins-Gitify/G41Fun
